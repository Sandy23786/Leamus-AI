    ...
    if (role === 'ai' && content.startsWith('__IMAGE_RESULT__')) {
      ...
      div.innerHTML = `
        ...
        <div class="msg-content" style="max-width:95%;">
          <div class="msg-bubble" style="padding:14px;">
            ...
            <div style="margin-bottom:12px;">
              <div style="font-size:13px;color:var(--text-secondary);margin-bottom:4px;">
                ✨ Generated <strong style="color:var(--text-primary);">"${data.prompt}"</strong>
              </div>
              <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
                <span style="background:rgba(108,127,255,0.15);border:1px solid rgba(108,127,255,0.3);border-radius:20px;padding:2px 10px;font-size:11px;color:var(--accent);">
                  🎨 Style: ${data.style}
                </span>
                <span style="font-size:11px;color:var(--text-muted);">4 variations · 1024px · Flux model</span>
              </div>
            </div>

            <!-- Main large image -->
            <div style="position:relative;border-radius:10px;overflow:hidden;border:1px solid var(--border);margin-bottom:10px;background:#111;" id="mainImgWrap_${Date.now()}">
              <img
                id="mainImg_${Date.now()}"
                src="${data.variations[0].url}"
                alt="Generated image"
                style="width:100%;display:block;cursor:zoom-in;max-height:420px;object-fit:contain;background:#0a0a0a;"
                onclick="window.open(this.src,'_blank')"
                onerror="this.style.minHeight='200px';this.insertAdjacentHTML('afterend','<div style=\'position:absolute;inset:0;display:flex;align-items:center;justify-content:center;color:var(--text-muted);font-size:13px;\'>⏳ Loading high-res image…</div>')"
              />
              <div style="position:absolute;top:8px;right:8px;display:flex;gap:6px;">
                <a href="${data.variations[0].url}" download="leamus-ai.jpg"
                  style="background:rgba(0,0,0,0.7);color:white;border-radius:8px;padding:5px 10px;font-size:12px;text-decoration:none;backdrop-filter:blur(4px);">
                  ⬇ Save
                </a>
                <span onclick="window.open('${data.variations[0].url}','_blank')"
                  style="background:rgba(0,0,0,0.7);color:white;border-radius:8px;padding:5px 10px;font-size:12px;cursor:pointer;backdrop-filter:blur(4px);">
                  🔍 Full size
                </span>
              </div>
              <div style="position:absolute;bottom:0;left:0;right:0;background:linear-gradient(transparent,rgba(0,0,0,0.6));padding:20px 10px 8px;">
                <div style="font-size:11px;color:rgba(255,255,255,0.7);">Click image for full size</div>
              </div>
            </div>
            ...
            <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin-bottom:10px;">
              ${data.variations.map((v, i) => `
                <div style="position:relative;border-radius:7px;overflow:hidden;border:2px solid ${i===0?'var(--accent)':'var(--border)'};cursor:pointer;transition:all 0.2s;"
                  class="img-thumb"
                  data-src="${v.url}"
                  data-download="${v.url}"
                  ...
                  <img src="${v.url}" alt="${v.label}"
                    ...
                    onerror="this.style.height='70px';this.style.background='#1a1a2e'"
                  />
                  <div style="position:absolute;bottom:0;left:0;right:0;background:rgba(0,0,0,0.6);padding:2px 4px;font-size:10px;color:white;text-align:center;">
                    ${v.label}
                  </div>
                </div>
              `).join('')}
            ...
            <details style="margin-top:6px;">
              ...
              <div style="margin-top:6px;padding:8px;background:rgba(0,0,0,0.2);border-radius:6px;font-size:11px;color:var(--text-secondary);line-height:1.5;">
                ${data.enhancedPrompt}
              </div>
            </details>
          ...
          <div class="msg-actions">
            <button class="msg-action-btn" onclick="window.open('${data.variations[0].url}','_blank')">🔍 View Full Size</button>
            <button class="msg-action-btn" id="dlAll_${Date.now()}">⬇ Download All</button>
          </div>
        </div>
      `;
      ...
      setTimeout(() => {
        const thumbs = div.querySelectorAll('.img-thumb');
        const mainImg = div.querySelector('[id^="mainImg_"]');
        thumbs.forEach(thumb => {
          thumb.addEventListener('click', () => {
            if (mainImg) {
              mainImg.src = thumb.dataset.src;
              mainImg.onclick = () => window.open(thumb.dataset.src, '_blank');
            }
            thumbs.forEach(t => t.style.borderColor = 'var(--border)');
            thumb.style.borderColor = 'var(--accent)';

            // Update save button
            const saveBtn = div.querySelector('[download]');
            if (saveBtn) saveBtn.href = thumb.dataset.download;
          });
        });

        // Download all
        const dlBtn = div.querySelector('[id^="dlAll_"]');
        if (dlBtn) {
